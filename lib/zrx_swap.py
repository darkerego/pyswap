        ...
        if resp.status_code == 200:
            ...
            if resp.status_code == 400:
                if not args.quote_only:
                    self._print.error('Probably needs approval')
                    ret = self.approve(sell_token)
                    self.poll_receipt(ret)
                    self.quote(buy_token, sell_token, raw_amount)

            self._print.warning(f'Non 200 response from 0x api: {resp}')
            return False
...
if __name__ == '__main__':
    ...
    if args.input_token and args.output_token:
        if not args.quote_only:
            ...
            api.quote(args.output_token, args.input_token, args.quantity, args.quote_only)
